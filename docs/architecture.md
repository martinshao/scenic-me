...
依赖方向：视图 → ViewModel → repository → service。当前首个本地演示切片由 `home` 和 `create` 两个功能组成，创作状态只在页面生命周期内存在，因此暂不创建无用 ViewModel、repository 或 domain 空类。接入相册、定位、持久化和 AI 服务时再按数据边界拆分。复杂共享业务出现后才加入 domain/use-cases；具体状态管理、路由、DI、数据库与后端仍未选库。

附近地图当前位于 `create/presentation/nearby_map_screen.dart`，使用 `CustomPainter` 和本地场景数据实现可运行视觉切片。城市、筛选和选择状态由创作页面持有，从地图进入肖像再返回时可以恢复。真实地图 SDK、POI repository、定位 service 与最近城市存储尚未创建；供应商和权限方案明确后再从该页面边界接入，避免演示实现提前绑定第三方。
